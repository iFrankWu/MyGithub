...
//				method = methods[i];
//			}
			//above code will exist a issue, if the filed have the same suffix char,such as id and pid
			if((prefix+fieldName).equalsIgnoreCase(methodName)){// so here, we should make sure the bean is the standard java bean
				method = methods[i];
				break;
			}
		}
